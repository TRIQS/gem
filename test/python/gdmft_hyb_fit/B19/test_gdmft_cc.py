...
import numpy as np
from triqs_ghostGA.solvers.pyscf_solvers import *
import os
...
class test_hemb_ci_1o3(unittest.TestCase):
    ...
    def test_gdmft_ci(self):
        ...
        fh5i = h5py.File(r'../D_Lamc_fit_hyb_B19.h5','r')
        D0 = fh5i['D'][...]
        Lambda_c0 = fh5i['Lambda_c'][...]
        D0 = np.kron(D0, np.eye(2))
        Lambda_c0 = np.kron(Lambda_c0, np.eye(2))
        fh5i.close() 
        ...
        edsolver = Pyscf_ccsd(ntot, nimp, nbath, restrict=True)
        gdmft = Gdmft(ntot, nimp, nbath, eks, eloc, Utensor, R=R0,
                      Lambda=Lambda0, D=D0, Lambda_c=Lambda_c0, edsolver=edsolver)
        gdmft.run_dmft_hyb(itmax=50, mix=0.5, tol=1e-4, beta=500,
                       silence=True, spin_pen=0.0, num_eig=10)

        oms = np.linspace(-5,5,200)
        gdmft.compute_Gf_Sig(0.0, eks, oms, 0.05)
        Gloc = np.sum(gdmft.Gf, axis=0)/gdmft.Gf.shape[0]

        np.savetxt('G_U%.2f_CCSD.dat'%(U), np.vstack((oms, Gloc[:,0,0].real, Gloc[:,0,0].imag)).T)
