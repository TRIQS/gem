'''Regression test of the ghost-GA cycle with SimpleED, two orbitals at B=3.

Kanamori interaction; the reference is the ``2o6_ci`` group of
``result_tests.h5``. Excluded from the test suite because of its cost.
'''
...
class test_hemb_2o6_simple_ed(unittest.TestCase):
    ...
    def test_scf_simple_ed(self):
        ...
        fragment = Fragment(nimp, nbath, eloc, Utensor, edsolver, verbose=0,
                            R=R0, Lambda=L0)
        _, docc, _ = scf.run_scf(lattice, fragment, itmax=9, mix=0.0, tol=1e-5, T=1e-3)
