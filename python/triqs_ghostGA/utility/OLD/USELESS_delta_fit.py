# Legacy mu_qp routines - never used, preserved for reference
...
# -------------------------------------------------
def get_mu_(eps, beta):
    # avoids overflow using where since else is zero
    def occupation_(mu,eps,beta):
        return np.sum( fermi_( eps-mu,beta))-len(eps)/2
    mu_min=np.min(eps)-10/beta
    mu_max=np.max(eps)+10/beta

    return brentq( occupation_, mu_min, mu_max, args=(eps,beta) )
...
# -------------------------------------------------
def dF_spectral_(H, dH, beta, tol=1e-14):
    eps, U = np.linalg.eigh(H)
    mu   = get_mu_(eps, beta)
    mu=0.0 # delete
    eps -= mu
...
# Residual
# ============================================================
def residual_LR_(x, beta, Lambda_c, D, F22_target, RTF12_target):
    '''
    Residual for Lambda+R fit.
    '''
    n, p = D.shape
    Lambda, R, mu = unpack_params_(x, n, p)

    H = build_H_(Lambda, Lambda_c, D, R)
    F = F_of_H_(H, beta).T
...
def residual_LcD_(x, beta, Lambda, R, F11_target, F12D_target):
    '''
    Residual for Lambda_c+D fit
    '''
    n, p = R.shape
    Lambda_c, D, mu = unpack_params_(x, n, p)

    H = build_H_(Lambda, Lambda_c, D, R,mu)
    F = F_of_H_(H, beta)
...
# ============================================================
def jacobian_LR_(x, beta, Lambda_c, D, F22_target, RTF12_target):
    '''
    ...
    for k in range(len(x)):
        ...
        dLambda, dR, dmu = unpack_params_(dx, n, p)
        dH = dH_dLambda_(dLambda, n) + dH_dR_(dR, D) - dmu*np.eye(2*n)
        dF = dF_spectral_(H, dH, beta)
...
def jacobian_LcD_(x, beta, Lambda, R, F11_target, F12D_target):
    '''
    ...
    for k in range(len(x)):
        ...
        dLambda_c, dD, dmu = unpack_params_(dx, n, p)
        dH = dH_dLambdac_(dLambda_c, n) + dH_dD_(dD, R) - dmu*np.eye(2*n)
        dF = dF_spectral_(H, dH, beta)
...
def solve_F_only_LR_(beta, Lambda_c, D, Lambda0, R0, mu0, F22_target, RTF12_target):
    '''
    Solve the root finding problem for a given set of Lambda_c,D,beta,F22=<badg b> and RTF12=R.T@<fdag b>
    using numerical derivatives
    '''
    x0 = pack_params_(Lambda0, R0, mu0)
    ...
    Lambda_sol, R_sol, mu_sol = unpack_params_(sol.x, Lambda0.shape[0], R0.shape[1])
    return sol, Lambda_sol, R_sol, mu_sol


# This one works
def solve_F_dF_LR_(beta, Lambda_c, D, Lambda0, R0, mu0, F22_target, RTF12_target):
    '''
...
def new_self_energy_( mu0,Lambda0,R0, Lambda_c,D, F22_target,RTF12_target, beta=200, method="dF"):
    print("New self-energy fitting Lambda and R")
    if(method=="dF"):
        res, new_Lambda, new_R, new_mu = solve_F_dF_LR_(beta, Lambda_c,D,Lambda0,R0,mu0,F22_target,RTF12_target)
    elif(method=="F"):
        res, new_Lambda, new_R, new_mu = solve_F_only_LR_(beta, Lambda_c,D,Lambda0,R0,mu0,F22_target,RTF12_target)
    else:
...
# ============================================================
# This one works without derivatives and with least square instead of root
def solve_F_only_LcD_(beta, Lambda, R, Lambda_c0, D0, mu0, F11_target, F12D_target):
    x0 = pack_params_(Lambda_c0, D0, mu0)
    # We use least_squares because it supports 'trf' and '2-point' (numerical) jacobians
    sol = least_squares(
        residual_LcD_,
        x0,
...
# This one works
def solve_F_dF_LcD_(beta, Lambda, R, Lambda_c0, D0, mu0, F11_target, F12D_target):
    '''
    ...
    Lambda_c_sol, D_sol, mu_sol = unpack_params_(sol.x, D0.shape[0], D0.shape[1])
    return sol, Lambda_c_sol, D_sol, mu_sol


def new_hybridization_( mu0,Lambda_c0,D0, Lambda,R, F11_target, F12D_target, beta=200, method="dF"):
    print("New hybridization fitting Lambda_c and D")
    if(method=="dF"):
        res, new_Lambda_c, new_D, new_mu = solve_F_dF_LcD_(beta, Lambda,R,Lambda_c0,D0,mu0,F11_target,F12D_target)
    elif(method=="F"):
        res, new_Lambda_c, new_D, new_mu = solve_F_only_LcD_(beta, Lambda,R,Lambda_c0,D0,mu0,F11_target,F12D_target)
    else:
...
def residual_minimize_(x, beta, Lambda_c, D, F22_target, RTF12_target):
    r = residual_LR_(x, beta, Lambda_c, D, F22_target, RTF12_target)
    return 0.5 * np.dot(r, r)

def jacobian_minimize_(x, beta, Lambda_c, D, F22_target, RTF12_target):
    r = residual_LR_(x, beta, Lambda_c, D, F22_target, RTF12_target)
    J = jacobian_LR_(x, beta, Lambda_c, D, F22_target, RTF12_target)
    return J.T @ r

def solve_F_dF_minimize_(beta, Lambda_c, D, Lambda0, R0, F22_target, RTF12_target):
    x0 = pack_params_(Lambda0, R0)
    res = residual_LR_(x0, beta, Lambda_c, D, F22_target, RTF12_target)
    sol = minimize(
        residual_minimize_,
        x0,
